...
public class CarLogoRecognizer {
	...
	public CarLogoRecognizer(double maxError) {
		...
		ai = new MultiLayerPerceptron(TransferFunctionType.SIGMOID, ImageConverter.SIZE * ImageConverter.SIZE, 10, 3);
	}
	...
	protected void learn() throws VectorSizeMismatchException, IOException, InterruptedException {
		...
		while (!isTrained()) {
			...
			learnInNewThread(new LearningSet("merc01.jpg", 1, 0, 0), new LearningSet("merc02.jpg", 1, 0, 0),
					new LearningSet("mazda01.jpg", 0, 0, 1));
			learnInNewThread(new LearningSet("ford01.jpg", 0, 1, 0), new LearningSet("ford02.jpg", 0, 1, 0),
					new LearningSet("mazda02.png", 0, 0, 1));
			learnInNewThread(new LearningSet("merc03.jpg", 1, 0, 0), new LearningSet("merc04.png", 1, 0, 0),
					new LearningSet("mazda03.jpg", 0, 0, 1));
			learnInNewThread(new LearningSet("ford03.png", 0, 1, 0), new LearningSet("ford04.jpg", 0, 1, 0),
					new LearningSet("mazda04.png", 0, 0, 1));
	...
	private boolean isTrained() throws IOException {
		double error = getErrorForSets(new LearningSet("merc01.jpg", 1, 0, 0), new LearningSet("ford01.jpg", 0, 1, 0),
				new LearningSet("mazda01.jpg", 0, 0, 1), new LearningSet("merc02.jpg", 1, 0, 0), new LearningSet("ford02.jpg", 0, 1, 0),
				new LearningSet("mazda02.png", 0, 0, 1), new LearningSet("merc03.jpg", 1, 0, 0), new LearningSet("ford03.png", 0, 1, 0),
				new LearningSet("mazda03.jpg", 0, 0, 1), new LearningSet("merc04.png", 1, 0, 0), new LearningSet("ford04.jpg", 0, 1, 0),
				new LearningSet("mazda04.png", 0, 0, 1));
	...
	private double getErrorForSet(LearningSet set) throws IOException {
		...
		for (int i = 0; i < 3; i++) {
			lastError = Math.abs(set.getOutputValues()[0] - answer.getData()[0]);
			if (lastError > error) {
				error = lastError;
			}
		}
	...
	private void learnInNewThread(LearningSet... set) throws IOException {
		DataSet trainingSet = new DataSet(ImageConverter.SIZE * ImageConverter.SIZE, 3);
